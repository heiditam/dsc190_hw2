...
### Methods
First, we find the indices of each responder group.
```{r}
males <- as.numeric(rownames(video_data[video_data$sex == 1, ]))
females <- as.numeric(rownames(video_data[video_data$sex == 0, ]))

computer <- as.numeric(rownames(video_data[video_data$own == 1, ]))
no_computer <- as.numeric(rownames(video_data[video_data$own == 0, ]))
```